...
<template>
  <div class="flex w-full flex-1 items-center justify-center px-4 sm:px-6">
    <LoadingState v-if="isLoading" />

    <div
      v-else
      class="flex w-full max-w-7xl flex-col items-center gap-6 sm:gap-8"
    >
      <div class="flex w-full flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
        <h2 v-if="!searchQuery.trim()" class="text-left text-2xl font-bold text-slate-900 sm:text-3xl md:text-4xl">Latest Travel Guides</h2>
        ...
        <div class="relative w-full sm:w-auto">
          ...
          <input id="trip-search" v-model="searchQuery" type="search" placeholder="Search for trips..." class="w-full border border-slate-200 bg-white/90 pl-10 pr-5 py-3 text-sm text-slate-700 shadow-[0_20px_40px_rgba(15,23,42,0.06)] outline-none transition-all duration-200 focus:border-sky-300 focus:ring-2 focus:ring-sky-200 sm:w-64 md:w-72" />
        </div>
      </div>
      ...
      <template v-if="hasBaseTrips">
        <p
          ...
          class="w-full rounded-2xl border border-slate-200 bg-white/80 px-5 py-6 text-center text-sm text-slate-500 shadow-sm sm:px-6"
        >
